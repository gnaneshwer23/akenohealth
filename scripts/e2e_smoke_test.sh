...
consent_token="consent:$(python3 - <<'PY'
import time
...
echo "2) Sending ingestion payload..."
ingestion_response="$(curl -fsS -X POST "${BASE_INGESTION_URL}/ingest" \
  ...
  -d "{
    ...
    \"payload\": {\"resourceType\": \"Observation\", \"id\": \"obs-smoke\"}
  }")"
...
echo "4) Posting governance decision..."
